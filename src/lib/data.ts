...
function getCatalogUrl(language: LanguageCode): string {
  return withBase(`data/${language}.json`);
}

function getResourcesUrl(): string {
  return withBase('data/resources.json');
}
...
export async function loadCatalog(language: LanguageCode, useCache = true): Promise<CatalogCategory[]> {
  ...
  const response = await fetch(getCatalogUrl(language));
  if (!response.ok) {
...
export async function loadResourceCatalog(): Promise<Map<string, ServiceResourceEntry>> {
  const response = await fetch(getResourcesUrl());
  if (!response.ok) {
